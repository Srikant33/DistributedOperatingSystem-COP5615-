...
type AesEncryption(?mode:string, ?size:int) = 
    ...
    /// This key will be used to create the encryption and authentication keys.
    /// </summary>
    /// <param name="keyLen">reqional, the key size.</param>
    /// <param name="raw">reqional, returns raw bytes, not base64-encoded.</param>
    ...
    member private this.Keys(salt:byte[], ?pwd:string) = 
        ...
        if pwd <> null then
            dkey <- this.Pbkdf2Sha512(pwd, salt, keyLen + macKeyLen)
    ...
    member private this.Pbkdf2Sha512(pwd:string, salt:byte[], dkeyLen:int):byte[] =
...
let akka_user_register (mailbox: Actor<_>) = 
    let rec loop () = actor {
        ...
        let cmd = req+","+POST+","+user_name+","+pwd+","+target_user_name+","+tweet_content+","+hash_mentioned+","+at+","+register
...
let akka_client_simulator (mailbox: Actor<_>) = 
    let rec loop () = actor {        
        ...
        | :? string   ->
            ...
            let mutable user_name = "user"+(string idx)
            let mutable pwd = "pwd" + (string idx)
            let mutable target_user_name = "user"+rand.Next(N) .ToString()
...
//            let cmd = "send, ,user"+(string i)+",pwd"+(string i)+", ,t, , , "
    ...
    printfn "The time of send 10 msg is %f" time_send
